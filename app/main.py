...
def normalize_role_display(role: str) -> str:
    r = (role or "").strip()
    rl = r.lower()
    if "team leader" in rl or "teamleader" in rl:
        return "TeamLeader"
    return r
...
add_column_if_missing("attendance", "weekend_hours", "REAL", "0")  # vikendove hodiny
...
def load_layout(tab_key: str):
    row = df_query("SELECT columns_json FROM ui_layout WHERE tab_key=?", (tab_key,))
    if row.empty:
        return None
    try:
        return json.loads(row.iloc[0]["columns_json"])
    except Exception:
        return None

def save_layout(tab_key: str, columns: list[str]):
    exec_query(
        """INSERT INTO ui_layout(tab_key, columns_json, updated_at)
        VALUES(?,?,datetime('now'))
        ON CONFLICT(tab_key) DO UPDATE SET
          columns_json=excluded.columns_json,
          updated_at=datetime('now')
        """,
        (tab_key, json.dumps(columns, ensure_ascii=False)),
    )

def clear_layout(tab_key: str):
    exec_query("DELETE FROM ui_layout WHERE tab_key=?", (tab_key,))

def apply_layout(df: pd.DataFrame, tab_key: str) -> pd.DataFrame:
    saved = load_layout(tab_key)
    if not saved:
        return df
    cols = list(df.columns)
    ordered = [c for c in saved if c in cols] + [c for c in cols if c not in saved]
    return df[ordered]

def layout_controls(tab_key: str, df: pd.DataFrame, label: str = "Rozložení sloupců"):
    with st.expander(label, expanded=False):
        current_cols = list(df.columns)
        saved = load_layout(tab_key) or current_cols
        # keep only existing
        saved = [c for c in saved if c in current_cols]
        default = saved + [c for c in current_cols if c not in saved]
        new_order = st.multiselect("Pořadí sloupců (vyber v pořadí)", options=current_cols, default=default)
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Uložit rozložení", key=f"save_layout_{tab_key}"):
                final = list(new_order) + [c for c in current_cols if c not in new_order]
                save_layout(tab_key, final)
                st.success("Rozložení uloženo.")
        with c2:
            if st.button("Obnovit rozložení", key=f"clear_layout_{tab_key}"):
                clear_layout(tab_key)
                st.success("Rozložení obnoveno na výchozí.")


# months defaults
add_column_if_missing("months", "afternoon_pct", "REAL", "0.10")
add_column_if_missing("months", "weekend_pct", "REAL", "0.10")  # priplatek vikend (min 10% dle ZP)
...
def upsert_attendance(month_key, employee_id, hours_worked, overtime_hours, overtime_comp_hours,
                     afternoon_hours, weekend_hours, holiday_days, holiday_hours, vacation_days, sick_days, unpaid_days):
    exec_query("""
    INSERT INTO attendance(
        month_key, employee_id,
        hours_worked, overtime_hours, overtime_comp_hours,
        afternoon_hours, weekend_hours, holiday_days, holiday_hours,
    ...
    """, (
        ...
        float(afternoon_hours), float(weekend_hours), float(holiday_days), float(holiday_hours),
...
def role_group(role: str) -> str:
    """Map role text to evaluation group keys."""
    r = (role or "").strip().lower()

    # Team leader (normalize)
    if "team leader" in r or "teamleader" in r or r.startswith("teamleader") or r.startswith("team leader"):
        return "TEAMLEADER"

    # Vedoucí výroby
    if "vedouc" in r and "vyrob" in r:
        return "VEDOUCI"
    ...
    # Kontrola kvality
    if "kontrol" in r and "kvalit" in r:
        return "KVALITA"
...
VAR_WEIGHTS = {
    ...
    # C) Team leader (15/20/5 = 40 %)
    "TEAMLEADER": {"TL_QUAL": 0.15, "TL_ORG": 0.20, "TL_BOZP": 0.05},
...
LABEL_MAP = {
    ...
    "TL_QUAL": "Kvalita výroby a dohled nad týmem",
    "TL_ORG": "Organizace směny a plnění plánu",
    "TL_BOZP": "Dodržování pracovního řádu, BOZP a pořádku",
}
...
def export_ucetni_podklad_xlsx(df_odprac: pd.DataFrame, df_nahrady: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(bio, engine="openpyxl") as writer:
        ...
        # Formát čísel: 2 desetinná místa (průměrná hodinovka a hodinové sazby)

        def _fmt2(ws, headers_set):
        ...
        _fmt2(writer.sheets["PODKLAD_ODPRAC"], {"Hodinovka 100%","Nároková Kč/h (60%)","Nenároková max Kč/h (40%)","Nenároková vyplac. Kč/h","Vyplacená Kč/h celkem"})

        _fmt2(writer.sheets["PODKLAD_NAHRADY"], {"Průměr náhrady (Kč/h)"})
...
with tabs[1]:
    ...
    """)
    # Průměrná hodinovka: vždy 2 desetinná místa
    if "Prumerna_hodinovka" in emp.columns:
        emp["Prumerna_hodinovka"] = pd.to_numeric(emp["Prumerna_hodinovka"], errors="coerce").fillna(0.0).round(2)
    ...
    emp_disp = apply_layout(emp, 'zamestnanci')
    layout_controls('zamestnanci', emp_disp)
    edited = st.data_editor(emp_disp, width="stretch", hide_index=True,
                            disabled=["id","Jméno","Pozice","Narokova_hodinovka_60","Nenarokova_hodinovka_40_max"])
    if st.button("Uložit hodinovky/extra bonus"):
        for _, r in edited.iterrows():
            exec_query("UPDATE employees SET hourly_rate=?, hourly_rate_avg=?, allow_extra_bonus=? WHERE id=?",
                       (float(r["Hodinovka_Kc_h"]), round(float(r.get("Prumerna_hodinovka", r["Hodinovka_Kc_h"])), 2), int(r["Extra_bonus_povolen"]), int(r["id"])))
        st.success("Uloženo.")
    st.divider()
...
with tabs[2]:
    ...
    att = df_query("""
        SELECT e.id, e.full_name AS Jméno, e.role AS Pozice,
          ...
          COALESCE(a.weekend_hours,0) AS Vikendove_hodiny,
    ...
    att_disp = apply_layout(att, 'dochazka')
    layout_controls('dochazka', att_disp)
    edited = st.data_editor(att_disp, width="stretch", hide_index=True, disabled=["id","Jméno","Pozice"])
    if st.button("Uložit docházku"):
        for _, r in edited.iterrows():
            upsert_attendance(month_key, int(r["id"]), r["Odpracovane_hodiny"], r["Prescasy"], r["Prescasy_nahradni_volno"],
                              r["Odpoledni_hodiny"], r["Vikendove_hodiny"], r["Svatky"], r["Svatky_hodiny"], r["Dovolena_dny"], r["PN_dny"], r["Neplacene_dny"])
...
with tabs[3]:
    ...
    # Zobrazení jmen ve formátu 'Příjmení Jméno' a řazení dle příjmení
    if "Jméno" in grid.columns:
        grid["Jméno"] = grid["Jméno"].map(format_name_surname_first)
    elif "full_name" in grid.columns:
        grid["Jméno"] = grid["full_name"].map(format_name_surname_first)
        # ponecháme full_name pro interní použití, ale zobrazovat budeme Jméno
    grid = sort_df_by_surname(grid, name_col="Jméno") if "Jméno" in grid.columns else grid
    grid["score"] = grid["score"].fillna(1.0)
    grid_disp = apply_layout(grid, 'hodnoceni')
    layout_controls('hodnoceni', grid_disp)
    edited = st.data_editor(grid_disp[["id","Jméno","Pozice","Oblast","area_code","score"]].rename(columns={"score":"Hodnocení (1.0/0.5/0.0)"}),
...
with tabs[5]:
    ...
    data = df_query("""
        SELECT e.id, e.full_name, e.role, e.coeff, e.counts_to_normative, e.receives_prod_bonus,
               ...
               COALESCE(a.weekend_hours,0) AS weekend_hours,
    ...
    data["premium_weekend"] = (data["avg_rate_used"] * pd.to_numeric(data.get("weekend_hours",0.0), errors="coerce").fillna(0.0) * (float(getattr(mp, 'weekend_pct', 0.10)) if float(getattr(mp, 'weekend_pct', 0.10))>0 else 0.10)).round(2)
    ...
    # Normalize role display (TeamLeader)


    if "role" in data.columns:


        data["role"] = data["role"].map(normalize_role_display)
    ...
    view = apply_layout(view, 'vysledky')
    layout_controls('vysledky', view)
    ...
    try:
        ...
        avg_rate = pd.to_numeric(data.get("avg_hourly_rate", data.get("hourly_rate_avg", data.get("avg_rate_used", h100))), errors="coerce").fillna(0.0)
        ...
        df_nahrady = pd.DataFrame({
            ...
            "Víkendové hodiny": pd.to_numeric(data.get("weekend_hours",0.0), errors="coerce").fillna(0.0).round(2),
            "Příplatek víkend 10% (Kč)": (avg_rate * pd.to_numeric(data.get("weekend_hours",0.0), errors="coerce").fillna(0.0) * (float(getattr(mp, 'weekend_pct', 0.10)) if float(getattr(mp, 'weekend_pct', 0.10))>0 else 0.10)).round(2),
